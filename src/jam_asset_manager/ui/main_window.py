...
def get_project_path(name, config):
    project = config.project(name)
    return str(project.root) if project is not None else ""


def get_episode_path(name, config):
    project = config.project(name)
    return project.episode_path if project is not None else ""


def get_asset_path(name, current_project, config):
    project = config.project(current_project)
...
def is_allowed_asset(name, config):
    extensions = config.application.allowed_extensions
    return is_allowed_extension(name, extensions)


def get_asset_records(path, config):
...
def get_episode_records(project_name, config):
    project = config.project(project_name)
    return get_episodes(project) if project is not None else []


def get_scene_records(episode, project_name, config):
    project = config.project(project_name)
    excluded_names = config.application.excluded_names
...
class MainWindow(QMainWindow):
    ...
    def is_asset_directory(self, name):
        return any(
            is_path_within(
                name,
                os.path.join(self.get_current_project_path(), asset_type.relative_path),
            )
            for asset_type in self.get_current_asset_types()
        )
    ...
    def add_message_to_report(self, path, hours, text):
        ...
        self.update_report_note()

    def get_current_project_name(self):
        return self.ui.comboBox_projName.currentText()

    def get_current_project_path(self):
        return get_project_path(self.ui.comboBox_projName.currentText(), self.config)
        ...
        if self.is_asset_directory(dir_path):
            ...
            if os.path.isdir(dir_path):
                for filename in sorted(os.listdir(dir_path)):
                    ...
                    if (
                        is_allowed_asset(path, self.config)
                        and os.path.normcase(os.path.splitext(path)[0]) == target_stem
                    ):
                        self.ui.listWidget_assets.clear()
                        self.create_asset_item(AssetRecord(element_name, dir_path, path))
            ...
            self.create_scene_item(outline)
    ...
    def get_render_template_path(self):
    ...
    def update_asset_types(self):
        ...
        self.ui.comboBox_aTypes.addItems(
            [asset_type.name for asset_type in self.get_current_asset_types()]
        )
    ...
    def update_asset_tree(self):
        ...
        self.update_assets()
        self.save_selection_state("asset")

    def update_episode_list(self):
        self.ui.listWidget_episodes.clear()
        episodes = get_episode_records(self.ui.comboBox_projName.currentText(), self.config)
    ...
    def create_asset_item(self, item):
    ...
    def populate_scenes(self, scenes):
    ...
    def update_scenes(self):
        ...
        self.filter_scenes()
        self.save_selection_state("episode")
    ...
    def update_assets(self):
        ...
        search_path = selected_path or get_asset_path(
            self.ui.comboBox_aTypes.currentText(),
            self.ui.comboBox_projName.currentText(),
            self.config,
        )
        self.asset_list[:] = get_asset_records(search_path, self.config)
        self.filter_assets()
    ...
    def create_scene(self):
        ...
        created = maya_scenes.create_render_scene(
            item.name,
            item.animation_path,
            item.render_path,
            self.get_render_template_path(),
        )
        if created:
    ...
    def open_selected(self):
        ...
        if data and os.path.isfile(data[1]):
            maya_scenes.open_render_scene(data[1])

    def update_scene(self):
        item = self._current_scene_data()
        if self.ui.tabWidget.currentIndex() != 1 or item is None:
            return
        if maya_scenes.update_render_scene(item.name, item.animation_path, item.render_path):
    ...
    def publish_element(self):
        ...
        if not path:
            return
        if self.is_scene_path(path) and maya_scenes.publish_scene():
            self.add_message_to_report(path, 0, "Published")
        elif self.is_asset_path(path) and maya_assets.publish_asset():
            self.add_message_to_report(path, 0, "Published")

    def import_element(self):
        data = self.get_selected_item_data()
        if data and self.is_asset_path(data[1]):
            maya_assets.import_asset(data[1])

    def notify_denoise_unavailable(self):
    ...
    def copy_outline_to_clipboard(self):
        copy_to_clipboard(self.ui.lineEdit_fullPath.text())
    ...
    def show_report_dialog(self):
        if self.get_selected_item_data():
            self.report_dialog = ReportDialog("report", self)
            self.report_dialog.show()

    def show_note_dialog(self):
        if self.get_selected_item_data():
            self.report_dialog = ReportDialog("note", self)
            self.report_dialog.show()

    def restore_asset_type_selection(self):
        current_index = self.ui.comboBox_aTypes.findText(self.get_current_asset_type())
        self.ui.comboBox_aTypes.setCurrentIndex(max(0, current_index))
    ...
    def update_all(self):
        ...
        self.update_assets()
        self.update_episode_list()
        self.restore_episode_selection()
        self.update_scenes()
        self.restore_asset_type_selection()
        self.save_selection_state("project")
    ...
    def _configure_window(self):
        ...
        control_icons = (
            ...
            (self.ui.actionNewScene, "new_scene.png"),
            (self.ui.actionOpen, "folder.png"),
            (self.ui.actionUpdate, "update.png"),
            (self.ui.actionPublish, "publish.png"),
            (self.ui.actionImport, "import.png"),
            (self.ui.actionCheck, "check.png"),
            (self.ui.actionDenoise, "denoise.png"),
        )
        for control, filename in control_icons:
            control.setIcon(_icon(filename, self.config.icons_path))
    ...
    def _connect_signals(self):
        ...
        self.ui.comboBox_projName.currentIndexChanged.connect(self.update_all)

        # update asset items when the folder changes
        self.ui.treeWidget_assetFolders.clicked.connect(self.update_assets)

        # update asset tree when the type changes
        self.ui.comboBox_aTypes.currentIndexChanged.connect(self.update_asset_tree)
        ...
        self.ui.listWidget_episodes.clicked.connect(self.update_scenes)

        # Open scenes and assets on a double-click.
        self.ui.tableWidget_scenesTable.doubleClicked.connect(self.open_selected)
        self.ui.listWidget_assets.doubleClicked.connect(self.open_selected)
        ...
        self.ui.toolButton_newScene.pressed.connect(self.create_scene)
        self.ui.toolButton_sCreate.pressed.connect(self.create_scene)
        self.ui.toolButton_update.pressed.connect(self.update_scene)
        self.ui.toolButton_sUpdate.pressed.connect(self.update_scene)
        self.ui.toolButton_publish.pressed.connect(self.publish_element)
        self.ui.toolButton_aPublish.pressed.connect(self.publish_element)
        self.ui.toolButton_sPublish.pressed.connect(self.publish_element)
        self.ui.toolButton_import.pressed.connect(self.import_element)
        self.ui.toolButton_aImport.pressed.connect(self.import_element)
        self.ui.toolButton_denoise.pressed.connect(self.notify_denoise_unavailable)
        self.ui.toolButton_check.pressed.connect(self.check_element)
        self.ui.toolButton_aRefresh.pressed.connect(self.refresh_assets)
        self.ui.toolButton_sRefresh.pressed.connect(self.refresh_scenes)
        self.ui.toolButton_goto.pressed.connect(self.go_to_asset)
        self.ui.toolButton_copyClipboard.pressed.connect(self.copy_outline_to_clipboard)
        self.ui.toolButton_addReport.pressed.connect(self.show_report_dialog)
        self.ui.toolButton_addNote.pressed.connect(self.show_note_dialog)
        # attach functions to menu bar
        self.ui.actionNewScene.triggered.connect(self.create_scene)
        self.ui.actionOpen.triggered.connect(self.open_selected)
        self.ui.actionUpdate.triggered.connect(self.update_scene)
        self.ui.actionPublish.triggered.connect(self.publish_element)
        self.ui.actionImport.triggered.connect(self.import_element)
        self.ui.actionCheck.triggered.connect(self.check_element)
        self.ui.actionDenoise.triggered.connect(self.notify_denoise_unavailable)
